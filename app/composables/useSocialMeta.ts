export const useSocialMeta = (options: { title: string, description: string, urlPath: string }) => {
  ...
  const fullUrl = `${config.public.siteUrl}${cleanPath === '/' ? '' : cleanPath}`
  ...
  useHead({
    ...
    meta: [
      ...
      { property: 'og:image', content: `${config.public.siteUrl}/portada/patirke121.jpg` },
      { name: 'twitter:card', content: 'summary_large_image' },
      { name: 'twitter:title', content: `${options.title} - Patirke Mendiguren` },
      { name: 'twitter:description', content: options.description },
      { name: 'twitter:image', content: `${config.public.siteUrl}/portada/patirke121.jpg` },
    ],
    link: [
